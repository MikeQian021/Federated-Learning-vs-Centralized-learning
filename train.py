...
parser.add_argument('--benchmark', type=str, default='cifar10_non_iid', help='what benchmark / dataset to run on')
...
parser.add_argument('--federated_method', type=str, default='FedAvg',
...
def init_data_generator_local_clients(args, seed):
    ...
    if 'toy_gaussians' in args.benchmark:
        ...
        #修改1
        if 'mid_iid' in args.benchmark:
            data_split = 'mid_iid'
        elif 'mid_non_iid' in args.benchmark:
            data_split = 'mid_non_iid'    
        elif 'non_iid' in args.benchmark:
            data_split = 'non_iid'
        else:
            data_split = 'iid'
...
def upper_bound_batch_data(args, data_generator, seed):
    ...
    batch_worker = worker(model_parameters, optimiser_parameters, misc_parameters, trainloader, testloader,
                           client_ind=-1, wandb_log=args.wandb_log,
                           use_cuda=args.use_cuda, seed=seed)
    # Train batch worker
